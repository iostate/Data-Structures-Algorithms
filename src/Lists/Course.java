...
public class Course implements Serializable, Comparable<Course>
{
   ...
   //Programming project
   public int compareTo(Course element) {

       String other = element.getPrefix()+element.getNumber();
       String thisObject = this.getPrefix()+this.getNumber();
       int equal = 10;
       boolean notEqualFound = false;
       for (int scan = 0; scan < other.length(); scan++) {

           if ((int) thisObject.charAt(scan) == (int) other.charAt(scan)) {
               equal = 0;
               notEqualFound = false;
           } else if ((int) thisObject.charAt(scan) < (int) other.charAt(scan)) {
               equal = -1;
               notEqualFound = true;
               return -1;
